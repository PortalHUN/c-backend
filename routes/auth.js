...
router.route("/auth/login").post(async (req, res) => {
  ...
  await db.query(`SELECT u.Username, u.Password FROM users AS u WHERE u.Username = ${db.escape(username)};`, async (err, row) => {
    ...
    db.query(
      `SELECT RoleName AS role from roles as r
  ...
  jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET, async (err, decoded) => {
    ...
    db.query(`SELECT Username FROM users WHERE Username = ${db.escape(decoded.username)};`, (err, row) => {
      ...
      db.query(
        ...
        JOIN assignedroles AS ar ON r.ID = ar.RoleID
        JOIN users AS u ON u.ID = ar.UserID
        WHERE u.Username = ${db.escape(decoded.username)};`,
        (err, rows) => {
          if (err) return res.status(500).send(err);
